...
The repository is consolidated through `Phase 19T - Defenses cockpit playable foundation v1`.
...
- Ground Army still has a dedicated placeholder route that keeps its boundary explicit before specialized gameplay exists.
...
- Development-only seed profiles now provide the standard QA setup path for Galaxy, Planet, Construction, Research, Shipyard, Fleets, and Defenses without manual SQL.
- `minimal-validation` remains the deterministic shared baseline, `cockpit-validation` is now the accepted richer cross-cockpit baseline for Galaxy, Planet, Construction, Research, Shipyard, Fleets, and Defenses together, and the current cockpit-specific richer profiles are `shipyard-validation`, `fleet-validation`, `research-validation`, and `planet-full-validation`.
...
Current validated baseline after Phase 19T:

- backend: `dotnet build --no-restore` succeeded
- tests: `dotnet test --no-build` succeeded with `634` passing tests
...
- `cockpit-validation` now restores a non-empty, focusable, read-only Galaxy baseline alongside the accepted Planet, Construction, Research, Shipyard, Fleets, and Defenses cockpit flows.
