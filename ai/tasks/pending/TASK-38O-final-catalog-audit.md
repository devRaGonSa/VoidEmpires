...
## Goal
Audit the current catalog coverage and metadata readiness across buildings, research, ships, defenses, and resources.
...
9. Identify the current catalog source for each gameplay area and list missing metadata fields such as display names, descriptions, category, role, image keys, icon keys, sort order, and tags.
10. Document balancing gaps and explicitly defer final image generation.
...
- The audit identifies missing metadata and balancing gaps across all requested catalog groups.
- Deferred image generation is called out explicitly.
