...
## Goal
Document safe SQL Server connection setup using environment variables and user secrets instead of committed credentials.
...
9. Add documentation for `ConnectionStrings__DefaultConnection` and optional provider selection environment variables.
10. Include the safe SQL Server template connection string and explicit warnings not to commit secrets or real passwords.
...
- The safe connection string template is documented exactly with placeholders.
- Environment variable and `.NET user-secrets` guidance is included and contains no real password.
