...
## Goal
Document final database security recommendations for the user-managed SQL Server deployment.
...
9. Recommend least-privilege app credentials, strong passwords, encrypted connections, firewall or IP restrictions, and backup hygiene.
10. Explain why `sa` should be avoided in production where feasible and repeat the rule against committing credentials.
...
- Security notes cover least privilege, password strength, encryption, firewalling, backups, and credential hygiene.
- The guidance discourages `sa` for production use where possible.
