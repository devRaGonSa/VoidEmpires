...
## Goal
Run and record the final cross-stack validation gate for SQL Server preparation work.
...
9. Run backend, test, frontend, dev QA, lazy-import, copy-regression, and any newly added secret-scan validations required by the block.
10. Record the results clearly so closure tasks can summarize the final readiness state.
...
- Optional: docs/dev/final-db-readiness-report.md
...
- All required validation commands are executed and their outcomes are recorded.
- Any failure is documented with follow-up scope instead of being glossed over.
