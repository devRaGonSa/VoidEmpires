...
## Goal
Update deployment notes to reflect an external SQL Server final database target.
...
9. Document that the application connects to an external SQL Server using environment variables rather than embedded passwords.
10. Include NAS or deployment considerations and the `Encrypt=True;TrustServerCertificate=True;` expectation where relevant.
...
- docs/dev/product-readiness-report.md
- docs/dev/sql-server-runbook.md
- Optional: docker-related docs or compose templates
...
- Deployment notes reflect an external SQL Server connection model.
- Connection string and certificate guidance remain safe and credential-free.
