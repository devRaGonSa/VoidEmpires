...
---
id: TASK-38AL
title: Appsettings template cleanup
status: pending
type: backend
team: platform
...
## Goal
Clean appsettings templates so database configuration remains safe while documenting the final SQL Server target.
...
9. Replace or refine any unsafe or misleading database placeholders with safe templates only.
10. Document optional provider configuration while keeping environment-specific config safe and non-secret-bearing.
...
- Appsettings templates contain only safe placeholders and no real password.
- Optional provider configuration is documented cleanly.
