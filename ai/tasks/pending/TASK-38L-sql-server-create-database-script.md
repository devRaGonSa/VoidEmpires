...
## Goal
Add a safe manual SQL Server database creation script for SSMS use.
...
9. Create `scripts/sqlserver/create-database.sql` or an equivalent location with `IF NOT EXISTS` guards and comments for manual execution.
10. Use `VoidEmpires` as the default database name unless the script is intentionally parameterized and documented.
...
- The script creates the database safely with manual SSMS-oriented guidance.
- No login creation with real passwords is included; placeholders and comments only are allowed if needed.
