...
---
id: TASK-38AP
title: SQL Server migration dry-run docs
status: done
type: docs
team: platform
...
## Goal
Document the dry-run migration process for the final SQL Server workflow.
...
9. Explain how to generate a migration script, review it, apply it manually, verify the schema, run smoke validation, and reason about rollback or backup caveats.
10. Keep the process manual by default and make the no-auto-apply policy explicit.
...
- The dry-run process is fully documented from script generation through manual verification.
- Rollback and backup caveats are explicit.
