...
## Goal
Define how the repository should test SQL Server readiness without making normal test runs depend on the user's remote server.
...
9. Document how fast unit and integration tests remain provider-independent and how optional SQL Server smoke tests should be gated.
10. Describe skip or conditional patterns if SQL Server-specific smoke coverage is added.
...
- The strategy keeps normal `dotnet test` runs provider-independent.
- Optional SQL Server smoke testing is documented with explicit gating.
