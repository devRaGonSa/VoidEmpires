...
---
id: TASK-38AT
title: Repository secret scan lightweight
status: done
type: platform
team: platform
...
## Goal
Add a lightweight repository secret scan for docs, scripts, and config without external tooling dependencies.
...
9. Add a small PowerShell-based secret scan or extend the existing dev QA checks to inspect appsettings, docs, and scripts for obvious secrets.
10. Keep the scan deterministic, repository-local, and easy to run in CI or local validation.
...
- A lightweight secret scan exists without requiring external tools.
- The scan covers docs, scripts, and config files for obvious secret patterns.
