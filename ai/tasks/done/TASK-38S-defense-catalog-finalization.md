...
## Goal
Prepare defense catalog metadata for final database readiness without expanding gameplay mutations unsafely.
...
9. Add or refine defense metadata for category, role, description, future production requirements, future combat dependencies, image keys, icon keys, and tags.
10. Do not add defense production mutation unless the backend already supports it safely and tests cover the change.
...
- Defense metadata covers the requested readiness fields.
- No unsafe defense production mutation or combat execution is introduced.
