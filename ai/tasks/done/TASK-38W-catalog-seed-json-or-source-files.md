...
## Goal
Add structured catalog source files if the chosen seed architecture calls for them.
...
9. Create structured source files for buildings, research, ships, defenses, and resources using stable keys, descriptions, and image key placeholders if that is the selected architecture.
10. Keep binary assets out of the repository and ensure the new source files align with existing loading patterns.

## Files to read first
- AGENTS.md
- ai/tasks/pending/TASK-38V-seed-data-architecture.md
...
- Structured source files exist for the chosen catalog groups when the architecture requires them.
- Stable keys, descriptions, and image key placeholders are present and no binary images are committed.
