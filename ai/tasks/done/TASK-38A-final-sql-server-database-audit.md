...
## Goal
Audit the current persistence stack, provider assumptions, migrations, and catalog data sources before any SQL Server cutover work.
...
9. Document the current EF Core provider, package references, DbContext registrations, migration locations, connection string sources, DbSets, seed or catalog sources, and PostgreSQL-specific assumptions.
10. Capture SQL Server target constraints, unknowns, and cutover risks in a developer-facing audit document without changing runtime behavior.
...
- The audit identifies the current EF provider, connection string sources, migrations strategy, DbSets, catalog or seed mechanisms, PostgreSQL-specific code or types, and SQL Server target constraints.
- No behavior changes are introduced during the audit.
