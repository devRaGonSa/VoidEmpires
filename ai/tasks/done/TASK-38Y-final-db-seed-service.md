...
## Goal
Prepare an idempotent final catalog seed service that is safe for the final SQL Server phase.
...
9. Add or refine a service that seeds or updates catalog data idempotently without deleting user gameplay data.
10. Support dry-run behavior if feasible and ensure the service is not executed automatically against the user's real SQL Server.
...
- The seed service is idempotent and protects user gameplay data.
- Automatic execution against the real SQL Server is avoided.
