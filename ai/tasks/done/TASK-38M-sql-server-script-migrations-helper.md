...
## Goal
Add a PowerShell helper that generates a SQL Server migration script without applying it.
...
9. Add `scripts/sqlserver-script-migration.ps1` to generate an idempotent script to a local output path.
10. Print manual SSMS execution guidance and add parser or lint coverage in `check-dev-qa-scripts` if appropriate.
...
- The helper generates an idempotent SQL script without running `database update`.
- The helper does not require a password and explains manual execution in SSMS.
