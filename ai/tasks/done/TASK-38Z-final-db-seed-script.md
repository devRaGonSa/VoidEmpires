...
## Goal
Add a user-run PowerShell script for final catalog seeding with explicit safety warnings.
...
9. Create a script that requires an explicit environment or connection string context before it can seed catalog data.
10. Warn clearly that the action mutates the database, avoid embedding passwords, and support dry-run if the backend seed service allows it.

## Files to read first
- AGENTS.md
- ai/tasks/pending/TASK-38Y-final-db-seed-service.md
...
- The seed script requires explicit user intent and warns before database mutation.
- No password is committed or echoed and dry-run support is used if available.
