...
Write-Host "Action: Prepare Construction QA state"
...
try {
    ...
    Write-Host "powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\dev-qa-create-construction-order.ps1 -BaseUrl `"$($BaseUrl.TrimEnd("/"))`" -CivilizationId $targetCivilizationId -PlanetId $targetPlanetId"
}
catch {
