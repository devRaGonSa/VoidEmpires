...
- Current confirmation audit:
  - `/shipyard` now uses the reusable `GameModal` confirmation component.
  - The guarded enqueue path remains read-only until the modal primary action runs.
  - The current backend refresh audit and explicit acknowledgement remain required before submit.
