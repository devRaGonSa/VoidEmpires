...
public sealed class DevFleetActionManifestService : IDevFleetActionManifestService
{
    ...
    [
        ...
        Read("fleet.uiState.read", "Read fleet UI state", "GET", "/api/dev/fleets/ui-state", [Field("civilizationId", "Guid")], "Returns a UI-ready aggregate payload for fleet screen prototypes, including route/fuel capability hints but no destination-specific estimates."),
        Read("fleet.interception.readiness.read", "Read interception readiness", "GET", "/api/dev/strategic-map/interception-opportunities", [Field("civilizationId", "Guid")], "Cross-links the read-only interception readiness metadata for current transfer tooling. It does not execute interception, combat, damage, or interception resolution."),
        Action("fleet.travel.estimate", "Estimate orbital travel", "POST", "/api/dev/fleets/orbital-travel/estimate", true, 200, [400, 409, 503], [Field("civilizationId", "Guid"), Field("orbitalGroupId", "Guid"), Field("destinationPlanetId", "Guid")], "Read-only estimate of duration, resource costs, affordability, route profile, and placeholder fuel readiness. Use this when UI state has no destination context."),
    ...
    private static DevFleetActionManifestItem Action(
        string key,
        string displayName,
        string method,
        string route,
        ...
        IReadOnlyList<DevFleetActionFieldDto> fields,
        string notes) => new(key, displayName, method, route, isReadOnly, fields, successStatus, errorStatuses, notes);
